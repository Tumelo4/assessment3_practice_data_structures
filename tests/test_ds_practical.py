...
def test_calculate_tax_no_matching_category():
    ...
    assert calculate_tax(prods, "food") == 0
...
def test_deduplicate_records_basic():
    data = [{"id": 1, "name": "a"}, {"id": 1, "name": "a-dup"}, {"id": 2, "name": "b"}]
    assert deduplicate_records(data, "id") == [{"id": 1, "name": "a"}, {"id": 2, "name": "b"}]
 
def test_deduplicate_records_keeps_first_occurrence():
    # When two dicts share a key value, the earliest one in the list should win.
    data = [{"id": 1, "v": "first"}, {"id": 1, "v": "second"}]
    result = deduplicate_records(data, "id")
    assert result == [{"id": 1, "v": "first"}]
 
def test_deduplicate_records_no_duplicates():
    data = [{"id": 1}, {"id": 2}, {"id": 3}]
    assert deduplicate_records(data, "id") == data
 
def test_deduplicate_records_empty_list():
    assert deduplicate_records([], "id") == []
 
def test_deduplicate_records_all_same_key():
    data = [{"id": 5, "v": 1}, {"id": 5, "v": 2}, {"id": 5, "v": 3}]
    assert deduplicate_records(data, "id") == [{"id": 5, "v": 1}]
 
def test_deduplicate_records_missing_key():
    # Records without the dedup key at all are treated as distinct / passed through
    data = [{"x": 1}, {"x": 1}]
    assert deduplicate_records(data, "id") == [{"x": 1}, {"x": 1}]
 
def test_deduplicate_records_preserves_order():
    data = [{"id": 3}, {"id": 1}, {"id": 3}, {"id": 2}]
    assert deduplicate_records(data, "id") == [{"id": 3}, {"id": 1}, {"id": 2}]
...
def test_mask_sensitive_fields():
    ...
    masked = mask_sensitive(data, ("email", "id"))
    assert masked["email"] == "***"
